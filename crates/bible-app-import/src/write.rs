...
pub fn import_tsk(
    ...
    let module = pbcd::load_commentary(from, "TSK", &kjv.index, &kjv.books)?;
...
fn import_topic(
    ...
    let module = pbcd::load_topic(from, stem, &kjv.index, &kjv.books)?;
...
pub fn import_strongs(
    ...
    let mapped = pbcd::load_word_map(from, kjv)?;
    let lexicon = pbcd::load_lexicon(from)?;
...
fn import_lexicon(conn: &mut Connection, entries: &[pbcd::LexEntry]) -> Result<usize, WriteError> {
    if entries.is_empty() {
        return Ok(0);
    }
    let tx = conn.transaction()?;
...
fn import_commentary(
    ...
    let module = pbcd::load_commentary(from, stem, &kjv.index, &kjv.books)?;
