...
impl SimpleComponent for App {
    ...
    fn init(
        ...
        copy_menu.append(Some("Copy verse"), Some("win.copy-verse-here"));
        let copy_popover = gtk::PopoverMenu::from_model(Some(&copy_menu));
        copy_popover.set_has_arrow(false);
        copy_popover.set_parent(&model.chapter_view);
        ...
        let copy_on_destroy = copy_popover.clone();
        root.connect_destroy(move |_| {
            goto_on_destroy.unparent();
            copy_on_destroy.unparent();
        });
